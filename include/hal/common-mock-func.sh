...
# This is 2*1024*1024:
TEST_FLASH_CHIP_SIZE="${TEST_FLASH_CHIP_SIZE:-2097152}"
...
# does needed work. e.g. fsread_tool_test_mock for test tool.
  ...
  fsread_tool_${_tool}_mock $_args
...
fsread_tool_test_mock(){
  ...
  _args="$*"
  local _arg_d
  _arg_d="$(parse_for_arg_return_next -d $_args)"
  _arg_f="$(parse_for_arg_return_next -f $_args)"
